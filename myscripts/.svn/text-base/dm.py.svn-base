...
#
#
# Replace for dm - sending raw data to parser
#   (L) Sergey Sh
#
import select
import socket
...
class Client:
  ...
  def run(self):
    try:
      c = Sender(self.server.accept(), self.rhost, self.rport)
      c.start()
      self.threads.append(c)
      logging.debug("Successfully connect with new client thread! ")
    except Exception, e:
...
if __name__ == "__main__":
  ...
  parser.add_option("-l", action="store", type="int", default=7771, dest="local_port", help="Local port for listening")
  parser.add_option("-p", action="store", type="int", default=7771, dest="remote_port", help="Port of remote host connect to")
  parser.add_option("-h", action="store", type="str", default='vcentos', dest="remote_host", help="Remote host connect to")
  parser.add_option("-c", action="store", type="str", default='dm.conf', dest="config_path", help="Path to config, default: dm.conf")
  parser.add_option("-d", action="store_true", default=False, dest="debug", help="Debug mode on")
  
  (options, args) = parser.parse_args()
  logging.debug("Parameters: " + str([options.local_port, options.remote_port, options.debug]))
  if options.local_port and options.remote_port:
    print "Choose -l (server mode) or -p (client mode)"
    sys.exit(1)
  if not (options.remote_port and options.remote_host):
    print "Give -h remote host and -p remote port"
    sys.exit(1)
  
  if options.config_path and os.path.exists()
    conf = yaml.load
  
  if options.local_port:
    print "Server mode"
    logging.debug("Server mode")
    s = Server(options.local_port, )
    s.run()
  if options.remote_port:
    print "Client mode"
    logging.debug("Client mode")
    c = Client(options.local_port)
    c.run()